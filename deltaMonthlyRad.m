...
%             ar(h) = 1 + ((ChangeInRad(h)-ChangeInRad(h+12))/ChangeInRad(h));    % Here the file is assumed to include the baseline weather, plus only the new long-term mean values
            ...
            ar(h) = 1 + ((ChangeInRad(h+12)-ChangeInRad(h))/ChangeInRad(h));    % Here the file is assumed to include the baseline weather, plus only the new long-term mean values
            FRad(h) = ChangeInRad(h+12);
