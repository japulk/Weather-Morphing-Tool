function [ClimateVariables] = CalculateClimateVariables(Temperature, Solar_Radiation, WindSpeed, StartDate, EndDate)
...
for i =1:12
    ...
    ClimateVariables.MonthlyMeanWind(i) = mean(WindSpeed(Month==i));
end
%% Daily values
...
for j = 1:NboDays   % Loop through each day
    for k = Year(1):Year(end)
        ...
        ClimateVariables.MeanWind(j)    = mean(WindSpeed(DayYear == DayNumberYear(j) & Year == k));
    end
end
...
for m = 1:12
    ...
    ClimateVariables.Dailystd(m)        = std(ClimateVariables.DailyMean(DailyMonth == m));
end
