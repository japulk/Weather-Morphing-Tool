function [FutureHourlyData] = Morphing(ClimateVariables, delta, alfa, MorphingTechnique, Temperature, Solar_Radiation, WindSpeed, TimeVector, ar, aw)
...
CurrentWind         = WindSpeed;    % This is current wind speed
...
for i = 1:3     % 2 variables are morphed
    ...
    elseif i == 2            % 2nd loop is for radiation
        
        BaseData    = CurrentRadiation;
        
    else 
        
        BaseData = CurrentWind;
    ...
    switch(MorphingTech)
        ...
        case 'Stretch'
            ...
            for j = 1:12
                ...
                MorphedRadiation(Months==j) = BaseData(Months==j) * ar(j);
                
            end
            
            else
                
                MorphedWind = zeros(1,length(TimeVector));
                
                for j = 1:12
                    
                    MorphedWind(Months==j) = BaseData(Months==j) * aw(j);
...
FutureHourlyData.Wind           = MorphedWind;
