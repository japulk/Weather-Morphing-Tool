...
%                 WindDir = PathWind + FileNameWind +SaveFormat;
        ...
        switch(SaveFormat{m})
            ...
            case '.epw'
                ...
                if strcmp(FileExtensions{m}, '.epw')
                    
                    [~,~,~,~,~,~,~,~,tmy] = EPWreader(InputData.FileName.(FileNames{m}));
                    
                    % EPW_new.(EPWVaribles{m}) = Variables{m};
                    
                    MaxRow = size(tmy,1) - 8 ;      % From EPWreader function
                    
                    % Check that the size of the tmy file is the same as
                    % the .epw file tried to be created
                    
                    try 
                        
                        tmy(9:MaxRow+8,1) = cellstr(string(MorphingYear));
                        
                    catch
                        
                        errordlg('Ensure the Morphing Year value is integer')
                        
                    end
                        
                        if strcmp(Variables{m}, 'Temperature')
                            
                            try
                            
                                tmy(9:MaxRow+8,7) = cellstr(string(Temperature));
                        ...
                        elseif strcmp(Variables{m}, 'Solar_Radiation')
                            
                            try 
                                
                                tmy(9:MaxRow+8,14) = cellstr(string(Solar_Radiation));
                        ...
                        elseif strcmp(Variables{m}, 'WindSpeed')
                            
                            try 
                                
                                tmy(9:MaxRow+8,22) = cellstr(string(WindSpeed));
                        ...
                        writecell(tmy, FullFile, 'FileType', 'text')
                    
                else
                    
                    errordlg('Original File was not .epw, it makes it impossible to create one')
    ...
    case 'Single'
        ...
        switch(SaveFormat{1})
            ...
            case '.epw'
                ...
                if any(strcmp(FileExtensions, '.epw'))
                    
                    V1 = find(strcmp(FileExtensions, '.epw'), 1, 'first');
                    
                    [~,~,~,~,~,~,~,~,tmy] = EPWreader(InputData.FileName.(FileNames(V1)));
                    
                    [tmy]                 = EPWassignment(tmy, MorphingYear, Temperature, Solar_Radiation, WindSpeed);
                    
                    writecell(tmy, FullFileName, 'FileType', 'text')
                    
                    
                else
                    
                    errordlg('There is no original .epw file. Please select at least one .epw file as original file.', 'No .epw file found')
